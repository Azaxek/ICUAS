...
\begin{abstract}
Medical drone delivery systems typically optimize for minimum flight time or distance. However, in emergency medical logistics, time is merely a proxy for patient outcome. This paper proposes a paradigm shift from time-optimal routing to \textit{irreversible harm minimization}. We introduce the Harm-Aware Routing Engine (\HARE), a planning framework that explicitly models nonlinear, time-critical harm functions for medical payloads (e.g., blood, organs, AEDs). By defining patient-specific ``Points of No Return'' (PNR)---thresholds beyond which delivery delays cause irreversible morbidity or mortality---\HARE dynamically prioritizes routes and dispatch decisions. Simulation results demonstrate that while \HARE may incur marginally higher total flight times compared to traditional shortest-path algorithms, it significantly limits the occurrence of critical harm events and PNR violations, identifying an optimal fleet size of 7 units for the simulated urban sector and demonstrating robustness under PNR uncertainty.
\end{abstract}
...
\begin{itemize}
    ...
    \item \textbf{battery constraints}: max range $D_{max}$.
    \item \textbf{PNR soft constraints}: Ideally $A_i < t_{\PNR,i}$. If $A_i > t_{\PNR,i}$ is unavoidable, the system should prioritize optimizing other viable patients (triage).
\end{itemize}
...
\subsection{Greedy Harm-Gradient Algorithm}
Solving the full Harm-VRP is NP-hard (see Appendix). \HARE uses a heuristic approach for real-time dispatch:

\begin{algorithm}
\caption{\HARE Dispatch Heuristic}
\begin{algorithmic}[1]
...
Table \ref{tab1} summarizes the results of 100 mission simulations with a fleet size of 5 drones.

\begin{table}[htbp]
\caption{Simulation Results (N=100 Missions, 5 Drones)}
\begin{center}
\begin{tabular}{lccc}
\toprule
\textbf{Metric} & \textbf{FIFO} & \textbf{DistOpt} & \textbf{\HARE} \\
\midrule
Avg Flight Time (min) & 12.4 & \textbf{9.8} & 11.2 \\
Total Harm Score & 4580 & 4120 & \textbf{2850} \\
Lives Saved (Type A) & 62\% & 68\% & \textbf{89\%} \\
PNR Violations & 38 & 31 & \textbf{15} \\
\bottomrule
\end{tabular}
\label{tab1}
\end{center}
...
As shown, \HARE incurs a slightly higher average flight time than the distance-optimal strategy. However, it achieves a dramatic reduction in Total Harm Score and PNR violations. By prioritizing Type A patients who are approaching their PNR window, \HARE prevents irreversible outcomes that other algorithms ignore in pursuit of efficiency.
...
Assuming a drone sorty cost of \$50 (amortized hardware + energy + ops) and that a successful Type A (Cardiac) delivery saves an average of 5 QALYs (valued at \$50,000/QALY), the Benefit-to-Cost Ratio (BCR) is:
\begin{equation}
    BCR = \frac{\Delta \text{Survivors} \times \text{QALY} \times \$50,000}{\Delta \text{Flights} \times \$50}
\end{equation}
In our simulation, \HARE saved 27 more lives than FIFO over 100 missions (Table II). Even if \HARE incurs 10\% more "waste" flights due to aggressive pre-positioning, the BCR remains $>100:1$. This suggests that "wasting" drone battery to optimize for survival is not only ethically sound but economically dominant compared to the societal cost of preventable death.

\section{Discussion}

\subsection{Ethical AI and Human Factors}
The transition from FCFS to harm-based prioritization introduces profound ethical questions, a core theme for ICUAS '26. \HARE explicitly favors patients with acute conditions over those who waited longer but are stable. While this utilitarian approach maximizes aggregate survival ("lives saved"), it may lead to extended wait times for rural or Type C (routine) patients. This algorithmic bias must be transparent to human operators. Future iterations should incorporate "fairness constraints" to prevent indefinite starvation of low-priority requests, balancing clinical utility with equitable access.
...
\section{Hardware Implementation Proposal}
To validate the theoretical findings, we propose a reference architecture for a \HARE-enabled medical drone.

\subsection{Compute and Connectivity}
The \HARE algorithms are lightweight enough to run on edge compute modules. We specify the \textbf{NVIDIA Jetson Orin Nano} (40 TOPS) as the onboard flight computer. The "Harm Estimator" runs as a containerized microservice, capable of updating harm gradients at 10 Hz.
Connectivity is provided by a dual-redundant link: 5G/LTE for high-bandwidth telemetry and map updates, and a 915 MHz LoRa mesh radio for low-bandwidth "negotiation" messsages between drones in GPS-denied environments.

\subsection{Drone Platform}
The simulation assumes a quadrotor platform with specifications matching the \textbf{DJI Matrice 300 RTK}:
\begin{itemize}
    \item \textbf{Max Payload}: 2.7 kg (Sufficient for 2 units of Whole Blood or 1 AED).
    \item \textbf{Max Speed}: 23 m/s ($>$ 60 km/h simulation param).
    \item \textbf{Flight Time}: 55 mins (Empty), 30 mins (Max Load).
\end{itemize}
Integration with the PX4 Autopilot stack allows \HARE to send waypoints directly via MAVLink.
