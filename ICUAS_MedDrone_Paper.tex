...
\end{equation}
where $\alpha_i$ is a severity coefficient. Due to the scarcity of continuous clinical data for trauma transport, we approximate Type B (Hemorrhage) and Type C (Routine) as linear ($\beta=1$) and flat functions respectively, serving as strict lower bounds on urgency compared to cardiac arrest.
...
\end{equation}
Here, $K=100$ represents the maximum harm, and $k=0.5$ controls the steepness. These parameters are calibrated such that $H_i(t)$ approximates the 10\%-per-minute survival drop-off reported in out-of-hospital cardiac arrest studies \cite{larsen1993predicting}.
...
\STATE \quad \quad Compute score $S_i = \text{Grad}(H_i) \text{ [see Eq. 10]} \times w_{urgent} - \text{Penalty}(dist)$
...
\subsection{Setup}
We simulated a 10km $\times$ 10km urban service area with one central depot. \textbf{Environmental Constraints}: We incorporated stochastic wind variance ($\mu=10, \sigma=5$ km/h) and two static No-Fly Zones ($r=1.0$ and $r=1.5$ km) representing sensitive infrastructure. Patient requests were generated using a Poisson process ($\lambda = 5 \text{ to } 10$/hr). We compared three strategies:
\begin{itemize}
...
\textit{Optimality Check}: On small problem instances ($|R| \le 5$, $|V|=3$), we compared \HARE against a brute-force solver. \HARE consistently found solutions within 5\% of the global optimum.
...
The transition from FCFS to harm-based prioritization introduces profound ethical questions. \HARE explicitly favors patients with acute conditions over those who waited longer but are stable. Furthermore, the autonomous decision to 'abandon' a patient ($t > t_{PNR}$) raises complex legal questions regarding consent and autonomy. Does an algorithm have the authority to deny care based on probabilistic futility? Future iterations must incorporate "fairness constraints" and "Right to Try" overrides to prevent indefinite starvation of low-priority requests.

\subsection{Parameter Sensitivity and Limitations}
Simulation results indicate that \HARE maintains high survival rates ($>85\%$) even when the harm decay steepness $k$ varies from 0.2 to 1.0, suggesting the RHC controller is robust to modeling errors.
\textit{Limitations}: However, this study relies on synthetic survival curves. While grounded in literature \cite{larsen1993predicting}, real-world physiological responses are idiosyncratic. Furthermore, our environmental model assumes static NFZs; dynamic obstacles (birds, other aircraft) are not yet modeled.

\subsection{Future Work: Clinical Validation}
Future work will focus on integrating real-time vital sign telemetry from EMS dispatch centers to drive the harm estimator. Collaborative trials with medical professionals are essential to validate the safety of "autonomous triage" before operational deployment.
